...
class TagCoupon extends Base
{
    ...
    public function getList($page = 1, $pagesize = 10,$users_id = 0,$parse = [])
    {
        ...
        $pages = Db::name('shop_coupon')->where($map)->order($order)->paginate($pagesize, false, $paginate);
        $result = $pages->toArray();

        $result_coupon_ids = [];
        if (!empty($result['data'])) {
            foreach ($result['data'] as $k => $v) {
                $result['data'][$k]['start_time'] = date('Y/m/d');
                $result['data'][$k]['end_time'] = date('Y/m/d');
        ...
        if (!empty($users_id)) {
            ...
            $user_data = Db::name('shop_coupon_use')->where($use_where)->select();
            if (!empty($user_data)) {
    ...
    public function getGoodsList($param = array())
    {
        ...
        $coupon = Db::name('shop_coupon')->where('coupon_id',$param['coupon_id'])->find();

...
class ShopComment extends Base
{
    ...
    public function comment_list()
    {
        ...
        $New = get_archives_data($data, 'product_id');
        foreach ($data as $k => $v) {
            // 规格
        ...
        if ($this->usersTplVersion == 'v4') $eyou['SubmitUrl'] = url('user/ShopComment/product_comment_v4', ['_ajax'=>1]);
        $this->assign('eyou', $eyou);
        return $this->fetch('users/shop_comment_goods_list');
    }

    // 第四版会员中心 -- 添加评论
    public function product_comment_v4()
    {
        if (IS_AJAX_POST) {
            $post = input('post.');
            if (empty($post['order_id'])) $this->error('模板异常，缺少订单ID：order_id');
            if (empty($post['order_code'])) $this->error('模板异常，缺少订单编号：order_code');
            if (empty($post['newCommnet'])) $this->error('模板异常，缺少评价标识：newCommnet');
            ...
            foreach ($post['total_score'] as $key => $value) {
                if (!empty($key) && isset($value)) {
                    $insert[] = [
                        'product_id'  => !empty($post['product_id'][$key]) ? intval($post['product_id'][$key]) : 0,
                        'users_id'    => intval($this->users_id),
                        'order_id'    => intval($post['order_id']),
                        'order_code'  => strval($post['order_code']),
                        'details_id'  => !empty($post['details_id'][$key]) ? intval($post['details_id'][$key]) : 0,
                        'total_score' => !empty($value) ? intval($value) : 5,
                        'content'     => !empty($post['content'][$key]) ? serialize(htmlspecialchars($post['content'][$key])) : '',
                        'upload_img'  => !empty($post['upload_img'][$key][0]) ? serialize(implode(',', $post['upload_img'][$key])) : '',
                        'ip_address'  => clientIP(),
                        'is_new_comment' => 1,
                        'add_time'    => getTime(),
                        'update_time' => getTime()
            ...
            $resultID = Db::name('shop_order_comment')->insertAll($insert);
            if (!empty($resultID)) {
                // 商品主表增加评价数
                if (!empty($post['product_id'])) {
                    $where = [
                        'aid' => ['IN', array_merge($post['product_id'])],
                    ];
                    Db::name('archives')->where($where)->setInc('appraise', 1);
                }

                // 同步更新订单/商品为已评价
                if (!empty($post['details_id'])) {
                    $where = [
                        'details_id' => ['IN', array_merge($post['details_id'])],
                    ];
                    $update = [
                        'is_comment' => 1,
                        'update_time' => getTime()
                ...
                $this->success('评价成功！', url('user/ShopComment/index'));
            } else {
                $this->error('评价失败，请重试！');
            }
        }
    ...
    {
        if (empty($this->usersConfig['shop_open_comment'])) {
            $this->error('商品评价功能已关闭');
        }

...
class Shop extends Base
{
    ...
    public function shop_payment_page()
    {
        if (IS_POST) {
            ...
            $OrderData['logistics_type'] = intval($post['logistics_type']);
    ...
    public function to_split_goods()
    {
        ...
        foreach ($list as $key => $value) {
            ...
            if (!empty($value['spec_value_id']) && !empty($value['spec_price'])) $value['users_price'] = floatval($value['spec_price']);
            // 商品价格处理
            $value['subtotal'] = 0;
            if (!empty($value['users_price'])) {
                // 计算小计
                $value['subtotal'] = $value['users_price'] * $value['product_num'];
                $value['subtotal'] = floatval(sprintf("%.2f", $value['subtotal']));
            }
            // 虚拟商品
        ...
        if (IS_AJAX_POST) {
            ...
            foreach ($delivery as $key => $value) {
                ...
                $result[0]['amount'] = floatval(sprintf("%.2f", $result[0]['amount']));
                // 购物车ID
            ...
            foreach ($verify as $key => $value) {
                ...
                $result[1]['amount'] = floatval(sprintf("%.2f", $result[1]['amount']));
                // 购物车ID
    ...
    public function shop_add_address()
    {
        if (IS_AJAX_POST) {
            ...
            if (in_array($this->usersTplVersion, ['v4'])) unset($post['is_default']);
            ...
            if (!empty($addr_id)) {
                ...
                if (isMobile() || isWeixin()) {
                    ...
                    $this->success('添加成功！', session($this->users_id.'_EyouShopOrderUrl'), $resultData);
                }

                if (in_array($this->usersTplVersion, ['v4']) && !empty($post['type']) && 'list' == $post['type']) {
                    $resultData = [
                        'url' => url('user/Shop/shop_address_list'),
                        // 'url' => !empty($post['type']) && 'list' == $post['type'] ? url('user/Shop/shop_address_list') : url('user/Shop/shop_under_order'),
                    ];
                    $this->success('添加成功！', url('user/Shop/shop_address_list'), $resultData);
                }
    ...
    public function shop_edit_address()
    {
        ...
        if (in_array($this->usersTplVersion, ['v3', 'v4'])) {
...
EOF;
    ...
    public function get_pick_up_list()
    {
        $parse = input('post.');
        $page = !empty($parse['page']) ? $parse['page'] : 1;
        $pagesize = !empty($parse['pagesize']) ? $parse['pagesize'] : 10;
        $where['status'] = 0;
        $field = '*';
        $order = 'id desc';
        if (!empty($parse['lat']) && !empty($parse['lng'])) {
//            $res = Convert_GCJ02_To_BD09($parse['lat'], $parse['lng']);
            $lat = $parse['lat'];
            $lng = $parse['lng'];
...
//                $val['logo'] = get_default_pic($val['logo'],true);

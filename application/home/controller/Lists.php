        ...
        if (IS_POST && !empty($typeid)) {
            ...
            if (!empty($post['real_validate'])) {
                if (!empty($post['real_validate_phone_input']) && !empty($post['real_validate_attr_id'])) {
                    ...
                    if (empty($smsLog)) {
                        if ($this->home_lang == 'cn') {
                            $msg = '手机号码和手机验证码不一致，请重新输入！';
                        } else if ($this->home_lang == 'zh') {
                            $msg = '手機號碼和手機驗證碼不一致，請重新輸入！';
                        } else {
                            $msg = 'Mobile phone number and mobile phone verification code are inconsistent, please re-enter!';
                        }
                        $this->error($msg);
                    }
                    // 验证码判断
                    ...
                    if (1 === intval($smsLog['is_use']) || $smsLog['add_time'] <= $time) {
                        if ($this->home_lang == 'cn') {
                            $msg = '手机验证码已被使用或超时，请重新发送！';
                        } else if ($this->home_lang == 'zh') {
                            $msg = '手機驗證碼已被使用或超時，請重新發送！';
                        } else {
                            $msg = 'The mobile phone verification code has been used or timed out. Please resend it!';
                        }
                        $this->error($msg);
                    }
                    // 会员所有的未使用留言验证码设为已使用
                    ...
                    if ($this->home_lang == 'cn') {
                        ...
                        $msg = 'Please enter the mobile phone verification code!';
                    }
                    $this->error($msg);
                }
            }

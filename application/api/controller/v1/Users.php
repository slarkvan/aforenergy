...
class Users extends Base
{
        ...
        if (IS_AJAX) {
            ...
            if (is_dir('./weapp/DealerPlugin/')) {
                ...
                if (!empty($data['status']) && 1 == $data['status']) {
                    ...
                    $dealerPluginLogic->dealerAction();
                } else {
                    $this->error('请先启用分销插件！');
                }
            } else {
    ...
    public function points_shop_action()
    {
        if (IS_AJAX) {
            ...
            if (!empty($weappInfo) && 1 === intval($weappInfo['status'])) {
                // 调用分销逻辑层方法
                $pointsShopLogic = new \app\plugins\logic\PointsShopLogic($this->users);
                $pointsShopLogic->pointsShopAction();
            }
        }
        $this->error('请求错误！');
    }

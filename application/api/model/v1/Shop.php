...
//                        $list[$key]['product_spec'] .= $spec_value['spec_name'] . '：' . $spec_value['spec_value'] . '；';
    ...
    // 微信小程序支付
    private function GetWechatAppletsPay($OpenID = null, $out_trade_no = null, $total_fee = null, $notify_url = '')
    {
        $notify_url = !empty($notify_url) ? $notify_url : url('api/v1.Api/wxpay_notify', [], true, true, 1, 1);
        // 当前时间戳
        $time = time();
    ...
    private function GetBuyOrderList($CacheData = [], $users_id = null)
    {
        ...
        if (!empty($CacheData)) {
            ...
            $field = 'a.aid, a.typeid, a.title, a.litpic, a.users_price, a.users_discount_type, a.stock_count, a.prom_type, a.attrlist_id, b.value_id, b.aid as product_id, b.spec_value_id, b.spec_price, b.spec_stock';
            if ('v1.5.1' < getVersion()) {
            ...
            $list       = Db::name('shop_cart')->field('a.*,b.typeid, b.aid, b.title, b.litpic, b.users_price, b.users_discount_type, b.stock_count, b.prom_type, b.attrlist_id, c.spec_price, c.spec_stock, c.value_id')
                ->alias('a')
                ->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')
                ->join('__PRODUCT_SPEC_VALUE__ c', 'a.spec_value_id = c.spec_value_id and a.product_id = c.aid', 'LEFT')
                ->where($where)
...
//                $filter['is_comment'] = 0;
    ...
    public function orderReceipt($order_id, $users_id)
    {
        ...
        $orderStatus = Db::name('shop_order')->where($where)->getField('order_status');
    ...
    public function getOrderDetail($order_id, $users_id)
    {
        ...
        foreach ($result['goods'] as $key => $value) {
            ...
            // if ($virtual_delivery_status && !empty($virtual_delivery)) {
    ...
    /**************************     会员升级 开始    ****************************/
    // 会员升级
    public function upgradeUserLevel($param = [])
    {
        if (empty($param['type_id'])) $this->error('缺少升级类型');
        $users_type_manage = Db::name('users_type_manage')->where('type_id',$param['type_id'])->find();
        // 记录类型，消费
        $cause_type = 0;
        // 清理当前会员的未支付的充值记录
        $where = [
        ...
        $UsersMoney = $users['users_money'];
        $level_name = Db::name('users_level')->where('level_id',$users['level'])->value('level_name');
        // 充值记录数据处理
        $time = getTime();
        // 订单号规则
        $OrderNumber = date('Ymd') . $time . rand(10, 100);
        $pay_detail = "会员当前级别为【{$level_name}】，使用微信支付【{$users_type_manage['type_name']}】，支付金额为{$users_type_manage['price']}";
        // 记录数据
        $insert = [
            'users_id'      => $this->users_id,
            'cause_type'    => $cause_type,
            'cause'         => serialize($users_type_manage),
            'money'         => $users_type_manage['price'],
            'users_money'   => $UsersMoney - $users_type_manage['price'],
            'pay_method'    => 'wechat',
            'pay_details'   => serialize($pay_detail),
            'order_number'  => $OrderNumber,
            'level_id'  => $users_type_manage['level_id'],
            'status'        => 1,
            'add_time'      => $time
        ];
        ...
        if (!empty($insertID)) {
            if ('wechat' == $param['pay_type']) {
                // 微信支付
                $openid = Db::name('wx_users')->where('users_id', $param['users_id'])->getField('openid');
                $notify_url =  url('api/v1/Api/wxpay_notify_users', [], true, true, 1, 2);
                $Data = $this->GetWechatAppletsPay($openid, $OrderNumber,  $users_type_manage['price'],$notify_url);
                $ResultData = [
                    'WeChatPay' => $Data,
                    'OrderData' => [
                        'moneyid' => $insertID,
                ...
                //先判断余额够不够
                if ($users['users_money'] < $users_type_manage['price']){
                    $this->error('余额不足!');
                }
                // 余额支付
                $UpOrderData = [
                ...
                $ReturnID    = Db::name('users_money')->where('order_number', $OrderNumber)->update($UpOrderData);
                if (!empty($ReturnID)) {
                    $limit_arr = Config::get('global.admin_member_limit_arr');

                    $day = $limit_arr[$users_type_manage['limit_id']]['maturity_days'];
                    $users_update['level_maturity_days'] = Db::Raw('level_maturity_days +'.$day);
                    if (empty($users['level_maturity_days'])){
                        $users_update['open_level_time'] = getTime();
                    }
                    $users_update['level'] = $users_type_manage['level_id'];
                    $users_update['update_time'] = getTime();
                    $users_update['users_money'] = Db::Raw('users_money-'.$users_type_manage['price']);
                    Db::name('users')->where('users_id', $param['users_id'])->update($users_update);
    ...
    // 会员升级
    public function handleUpgradeUserLevel($param = [])
    {
        // 查询订单信息
        $where = [
            'cause_type' => 0,
        ...
        if (!empty($WeChatPay) && 'SUCCESS' == $WeChatPay['return_code'] && 'OK' == $WeChatPay['return_msg'] && 'SUCCESS' == $WeChatPay['trade_state']) {
            ...
            $ResultID = Db::name('users_money')->where($where)->update($update);
            if (!empty($ResultID)) {
                $usersMoney['cause'] = unserialize($usersMoney['cause']);
                $level_maturity_days = Db::name('users')->where('users_id', $param['users_id'])->value('level_maturity_days');
                $limit_arr = Config::get('global.admin_member_limit_arr');

                $day = $limit_arr[$usersMoney['cause']['limit_id']]['maturity_days'];
                $users_update['level_maturity_days'] = Db::Raw('level_maturity_days +'.$day);
                if (empty($level_maturity_days)){
                    $users_update['open_level_time'] = getTime();
                }
                $users_update['level'] = $usersMoney['cause']['level_id'];
                $users_update['update_time'] = getTime();
                Db::name('users')->where('users_id', $param['users_id'])->update($users_update);
                // 返回结束
                $this->success('支付完成');
    ...
    public function buyMedia($param = [])
    {
        ...
        $price = sprintf("%.2f",$list['users_price']*($users['discount']/100));

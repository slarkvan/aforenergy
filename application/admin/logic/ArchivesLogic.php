...
class ArchivesLogic extends Model
{
    ...
    public function batch_copy_all($lang = '',$is_jump = 0)
    {
        ...
        if (!empty($lang_channel_data)){
            foreach ($lang_channel_data as $k => $v){
                ...
                if ('images' == $v['nid']) {
                    ...
                    // 附件下载记录表
                    $downloadLogRow = Db::name('download_log')->where(['aid' => ['IN', $aids]])->delete();
        ...
        //新建数据开始
        $channel_ids = Db::name('archives')->where(['lang' => 'cn', 'is_del' => 0,'channel'=>['neq',6]])->group('channel')->field('channel')->select();
        $channel_ids = get_arr_column($channel_ids, 'channel');
        ...
        if (!empty($channeltypeRow)) {
            ...
            foreach ($channeltypeRow as $k => $v) {
                ...
                $aids = get_arr_column($archivesRow,'aid');
                $typeids = get_arr_column($archivesRow,'typeid');
                // 内容扩展表数据
                $tableExt = $v['table'] . "_content";
                $contentRow = Db::name($tableExt)->field('id', true)->where(['aid' => ['IN', $aids]])->getAllWithIndex('aid');
                $typeids_arr = [];

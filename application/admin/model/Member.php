...
{   
    ...
    public function batch_update_userslevel()
    {
        ...
        if (!empty($web_users_switch)) {
            ...
            /* END */
            $time = getTime();
            $where_str = "`level` <> {$level_id} AND `level_maturity_days` > 0 AND `level_maturity_days` < 36600 AND (`open_level_time` + (`level_maturity_days` * 86400)) <= {$time}";
            $r = Db::name('users')->where($where_str)->fetchSql(true)->update([
                    'level'           => $level_id,
                    'open_level_time' => 0,
